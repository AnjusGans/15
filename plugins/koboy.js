let handler = (m, { conn, usedPrefix, command, text }) => {
  conn.tembak = conn.tembak || { musuh: [], tembak: [] }
   if(/kiri/i.test(text)) {
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.sendButton(m.chat, pos, "Developed By Ajus", "Tembak", `${usedPrefix}${command} tembak`)
    return conn.send2Button(m.chat, pos, "Developed By Ajus", "←", `${usedPrefix}${command} kiri`, "→", `${usedPrefix}${command} kanan`)
