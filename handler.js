    ...
    esteh: 0,
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (xp > 200) m.reply('Ngecit Anjime') // Hehehe
          else m.exp += xp
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}blimit*`, m)
...
await conn.sendMessage(jid, { "contentText": action === 'add' ? '[ *WELCOME* ]' : '[ *Sayonara 👋* ]' , "footerText": text,
"buttons": [
...
"headerType": "DOCUMENT", "documentMessage": {
      ...
      case 'demote':
        ...
        if (chat.detect) this.send2ButtonLoc(jid, banp, text, 'Group Assistant', 'MENU', '#menu', 'OWNER', '#owner', null, {
          contextInfo: {
...
*@${m.participant.split`@`[0]} Telah Terdeteksi Menhapus Pesan
*Type*: ${Object.keys(m.message.message)[0]}

Untuk mematikannya atau ketik #disable delete
`.trim(), 'Group Assistant', 'DISABLE DELETE', '.disable delete', {
      quoted: m.message,
      contextInfo: {
  ...
  async onCall(json) {
    ...
    await this.sendMessage(from, 'Anda Telah Menelpon Bot, Jadi Kamu Akan TerBlock Otomatis', MessageType.extendedText)
...
Kamu Belum Terdaftar Dalam Database *Group Asisstant*
Jadi Daftar Terlebih Dahulu !
